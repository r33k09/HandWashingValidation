...
## Description
The program:
1. Processes images of hand washing steps (Classifying each step in groups)
...
# Montamos el drive para leer los contenidos de la base de datos
drive.mount('/content/drive')

# Importamos las funciones de mediapipe para la detección de manos y el trazado de dibujos sobre las manos
...
# Definimos una función para cargar los datos de todas las clases
def load_hand_data(data_folders):
    X, y = [], []
    
    # Asignamos una etiqueta numérica a cada clase (Step1=0, Step2=1, ..., Step7=6)
    for class_label, folder in enumerate(data_folders):
        if os.path.exists(folder):
            ...
            for filename in images:
                ...
                if landmarks is not None:
                    X.append(landmarks)
                    y.append(class_label)  # Usamos el número de clase como etiqueta

    return np.array(X), np.array(y)

# Agregamos las rutas de todos los pasos (7 clases)
data_folders = [
    "/content/drive/MyDrive/PRACTICAS LAVADO DE MANOS/Frames/Step1",
...
X, y = load_hand_data(data_folders)

# Verifica que haya datos antes de dividir
if len(X) > 0:
    # Convertir etiquetas a one-hot encoding para la salida multiclase
...
if len(X_train) > 0:
    # Definir el modelo de red neuronal para multiclase
    model = Sequential([
        ...
        Dense(7, activation='softmax')  # 7 clases (pasos del lavado de manos)
    ])
    ...
    model.save("handwash_multiclass_model.h5")
